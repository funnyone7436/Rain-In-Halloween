...
function BatsHtml({
  ...
  wobbleAmp = 0.8,
  wobbleFreq = 1.5,
  pixelWidth = 120, // size on screen (CSS pixels)
}) {
  const group = useRef()
  const refs = useRef([])
  ...
  useFrame(({ clock }) => {
    ...
    for (let i = 0; i < count; i++) {
      ...
      let p = ((t / secondsPerLap) * s.sway + s.phase) % 1
      if (!inwardOnly) p = 1 - Math.abs(1 - 2 * p)
      const e = 1 - Math.pow(1 - p, 2.2)

      const r = THREE.MathUtils.lerp(srcRadius, dstRadius, e)
      const yBase = THREE.MathUtils.lerp(startY, endY, e)
      const wobble = Math.sin((t + i * 0.4) * wobbleFreq) * wobbleAmp * (0.2 + 0.8 * (1 - e))
      const y = yBase + wobble

      const ang = s.ang + (1 - e) * 0.1 * s.spin
...
// helpers (top-level or near where you compute dynamic values)
	...
	// tweakables
	const RAIN_MIN_COUNT = 100;
	const RAIN_MAX_COUNT = 600;
	const RAIN_MIN_OP = 0.3;
        ...
        <Suspense fallback={null}>
		  ...
		  />
			...
			<BatsHtml
			  gifUrl={`${BASE}img/bat.gif`}
			  count={8}
			  srcRadius={outerMountainRadius}
			  dstRadius={imgRing.radius}
			  startY={20}
			  endY={10}
			  secondsPerLap={7.5}
			  inwardOnly
