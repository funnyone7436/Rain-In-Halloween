...
function MoonSystem({
  ...
  ringRadius = 56,
  ringSpinSpeed = 0.4,
  ringSpriteSize = [6,6,6,6,6,6], // per-sprite sizes (w1,w2,w3)
...
// === Motion → Moon params ===
	...
	const MOON_MAX  = 10;     // cap (tweak)
	const SPIN_BASE = 0.4;    // current default
	const SPIN_MAX  = 2;    // cap (tweak)
	...
	const { dynamicMoonSize, dynamicRingSpin } = React.useMemo(() => {
	  ...
	  const t = 1 - Math.exp(-m / 120);              // tweak 120 for sensitivity
