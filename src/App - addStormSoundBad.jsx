...
function LightningClassic({
  ...
  // NEW: called when a fresh strike is created
  onStrike, // (info: { position: THREE.Vector3, seed: number }) => void
}) {
  const mainGeo = React.useMemo(() => new THREE.BufferGeometry(), [])
  const thickMat = React.useMemo(
    () =>
  ...
  const build = () => {
    // advance seed and get a deterministic RNG for this strike
    const s = (state.current.strikeSeed = (state.current.strikeSeed + 1013904223) >>> 0 || 1)
    const rand = mulberry32(s)

    // choose endpoints that are mostly vertical
    let a, b, tries = 0
    while (true) {
      ;[a, b] = pickVerticalEndpointsRand(
        cloudY,
        cloudRadius,
    ...
    // position the point light near the middle of the bolt
    const mid = main[Math.floor(main.length / 2)]
    if (flash.current) flash.current.position.copy(mid)

    // notify the app so it can play sounds, etc.
    onStrike?.({
      position: mid.clone(),
      seed: state.current.strikeSeed,
    })
  }
  ...
  useFrame((_, dt) => {
    ...
    if (!s.active) {
      ...
      if (s.cooldown <= 0) {
        ...
        build() // ← triggers onStrike()
      }
    } else {
      s.t += dt
      const e = s.t / s.dur

      // quick, bright core + short afterglow
...
const onResults = React.useCallback((res) => {
  ...
  // tiny state we keep across calls
  let st = prevLmRef.current
  if (!st || typeof st !== 'object' || st.__v !== 2) {
    st = prevLmRef.current = { __v: 2, y: null, t: 0, out: 0, last: 0 }
  }

  // throttle to ~15 fps (keep if you like)
  const MIN_GAP_MS = 1000 / 15
  if (now - st.last < MIN_GAP_MS) return
  st.last = now

  // No landmarks -> hard zero
  if (!lm) { st.out = 0; onChange?.(0); return }

  // average visible hips (23, 24)
  const hips = [23, 24]
    .map(i => lm[i])
    .filter(p => p && (p.visibility ?? 0) >= 0.6)
  if (!hips.length) { st.out = 0; onChange?.(0); return }

  const y = (hips[0].y + (hips[1]?.y ?? hips[0].y)) / (hips[1] ? 2 : 1)

  // first frame
  if (st.y == null) {
    st.y = y; st.t = now; st.out = 0; onChange?.(0); return
  }

  const dt = Math.max(1e-3, (now - st.t) / 1000)

  // vertical speed (normalized units per second)
  const vy = (st.y - y) / dt;                 // up is positive (y decreases upward)

  // --- Simple “movement” metric ---
  const DEAD   = 0.002;                       // ignore micro jitter
  const SPEED  = Math.max(0, Math.abs(vy) - DEAD)
  const GAIN   = 1400;                        // scale to your preferred range
  const raw    = SPEED * GAIN

  // Fast response, slight smoothing (make ALPHA = 1 for totally raw)
  const ALPHA = 0.75                          // 0..1, higher = snappier
  st.out = ALPHA * raw + (1 - ALPHA) * st.out

  // If speed is tiny, hard-zero so it doesn’t hang
  if (SPEED <= 0) st.out = 0
...
function ScoreTicker({ gameActive, motionValue, scoreRef, setScore }) {
  useFrame((_, dtRaw) => {
    ...
    const dt = Math.min(dtRaw, 0.05)
    const energy = 1 - Math.exp(-Math.max(0, motionValue - 20) / 160) // 0..1
...
// helpers (top-level or near where you compute dynamic values)
	...
	// tweakables
	const RAIN_MIN_COUNT = 200;
	const RAIN_MAX_COUNT = 800;
	const RAIN_MIN_OP = 0.2;
	const RAIN_MAX_OP = 0.8;
	...
	const THUNDER_VOLUME = 0.1;
	
	// under your other refs/states near audioRef:
	const thunderClips = useMemo(() => ([
	  `${BASE}lightning.mp3`,
	  `${BASE}lightning1.mp3`,
	  `${BASE}lightning2.mp3`,
	]), [])

	// Simple “fire and forget” player using <audio>
	const playClip = React.useCallback(
	  (url, { volume = 0.8, rate = 1 } = {}) => {
		const a = new Audio(url)
		// ↓↓↓ the only important change
		a.volume = Math.max(0, Math.min(1, volume * THUNDER_VOLUME))
		a.playbackRate = rate
		a.play().catch(() => {})
	  },
	  []
	...
	// Called by LightningClassic on each new strike
	const handleLightningStrike = React.useCallback(({ position }) => {
	  // pick how many clips to play: 75% one clip, 25% two clips
	  const playTwo = Math.random() < 0.25
	  const idxs = [0,1,2].sort(() => Math.random() - 0.5)
	  const count = playTwo ? 2 : 1

	  for (let i = 0; i < count; i++) {
		const url = thunderClips[idxs[i]]

		// Optional: little randomness to make it feel alive
		const vol = 0.6 + Math.random() * 0.35   // 0.6 .. 0.95
		const rate = 0.9 + Math.random() * 0.2   // 0.9 .. 1.1

		playClip(url, { volume: vol, rate })
	  }
	}, [thunderClips, playClip])
