...
import { OrbitControls, useTexture, Billboard, Html } from '@react-three/drei'  // 👈 added Html
...
/* -------------------- Bat GIF pinned to a portrait -------------------- */
/**
 * Renders /img/bat.gif as an HTML element anchored in 3D above a portrait ring item.
 * - targetIndex: which portrait to attach to (0..N-1)
 * - It gently bobs up and down.
 */
function BatGif({
  src = `${BASE}img/bat.gif`,
  targetIndex = 0,            // 👈 pick the portrait index to attach the bat to
  ringRadius = 12,            // use imgRing.radius
  defaultY = 1.2,             // use imgRing.y as fallback
  items = null,               // pass imgItems for per-image y
  bobAmp = 0.35,
  bobSpeed = 1.5,
  yOffset = 2.4,              // height above the portrait
  pixelWidth = 96             // CSS pixel width of GIF
}) {
  const group = useRef()
  const count = Array.isArray(items) ? items.length : 0
  const positions = useMemo(() => ringPositions(count, ringRadius), [count, ringRadius])

  // base position above the selected portrait
  const base = positions[targetIndex] || [0, 0, 0]
  const baseY = (items?.[targetIndex]?.y ?? defaultY)

  useFrame(({ clock }) => {
    if (!group.current) return
    const t = clock.getElapsedTime()
    const y = baseY + yOffset + Math.sin(t * bobSpeed) * bobAmp
    group.current.position.set(base[0], y, base[2])
  })

  // <Html> renders DOM at a 3D position; sprite+center keeps it billboarded & centered
  return (
    <group ref={group}>
      <Html sprite center style={{ pointerEvents: 'none' }}>
        <img
          src={src}
          alt="bat"
          style={{
...
export default function App() {
  ...
  const portraitList = [
    ...
    { src: `${BASE}img/C3.png`, size: 32, y: 17 },
    ...
    <>
      ...
      >
        ...
        <Suspense fallback={null}>
          ...
          {/* 🦇 Bat GIF above a specific portrait (change targetIndex to pick one) */}
          <BatGif
            targetIndex={2}              // <-- choose which portrait index to attach to
            ringRadius={imgRing.radius}
            defaultY={imgRing.y}
            items={imgItems}
            src={`${BASE}img/bat.gif`}
            pixelWidth={96}
