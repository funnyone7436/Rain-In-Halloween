...
/* -------------------- Moon System (forwardRef) -------------------- */
const MoonSystem = React.forwardRef(function MoonSystem({
...
/* -------------------- Keep camera looking at moon -------------------- */
function LookAtMoon({ targetRef, yOffset = 0 }) {
  const { camera } = useThree()
  useFrame(() => {
    const g = targetRef.current
    if (!g) return
    const { x, y, z } = g.position
    camera.lookAt(x, y + yOffset, z)
  })
  return null
}
...
export default function App() {
  ...
  const [autoRotate] = useState(true) // not used now but you can wire it up
    ...
    <>
      ...
      >
        ...
        <Suspense fallback={null}>
          ...
          {/* 🌙 Moon with three orbiting sprites (forwardRef) */}
          <MoonSystem ref={moonRef} />

          {/* 👀 Keep the camera looking at the moon (adjust yOffset if you want above/below) */}
          <LookAtMoon targetRef={moonRef} yOffset={-10} />

          {/* 🦇 Bat GIF above a specific portrait */}
          <BatGif
            targetIndex={2}
        ...
        {/* Keep camera position fixed; prevent rotation so position won't change */}
        <OrbitControls
          enableRotate={false}
          enableZoom={false}
          enablePan={false}
