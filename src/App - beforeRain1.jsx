...
/* -------------------- Lightning (thicker via multi-copy jitter + brightness boost) -------------------- */
function Lightning({
  enabled = true,
  color = '#ffffff',
  cloudY = 10,
  ...
  lateralJitter = 0.2,
  branchLateral = 0.6,
  // NEW: brightness & thickness controls
  power = 6.0,          // multiplies line opacity (1–4 is typical)
  flashPower = 800,     // pointLight peak intensity
  copies = 8,           // total line copies (includes the 2 base lines)
  copyJitter = 0.05,    // how far each extra copy can deviate (world units)
  copyFade = 0.85       // opacity falloff per copy (0.7–0.95)
}) {
  ...
  // Extra geometries for “thickness”
  const extraGeos = useMemo(
    () => Array.from({ length: Math.max(0, copies - 2) }, () => new THREE.BufferGeometry()),
    [copies]
  )
  ...
  // - makeBoltPoints(a,b)
  // - enforceDownward(pts)
  // - makeBranch(points, startIndex, ...)
  // - pickVerticalEndpoints(...)

  const jitterPoints = (pts, amt) =>
    pts.map(p =>
      new THREE.Vector3(
        p.x + (Math.random() * 2 - 1) * amt,
        p.y + (Math.random() * 2 - 1) * amt * 0.35, // keep mostly vertical
        p.z + (Math.random() * 2 - 1) * amt
      )
    )
  ...
  const build = () => {
    // Force vertical-ish column
    let a, b, tries = 0
    while (true) {
      ;[a, b] = pickVerticalEndpoints(cloudY, cloudRadius, { minHeight, maxHeight, lateralJitter })
    ...
    setAndBound(b1Geo, br1)
    setAndBound(b2Geo, br2)

    // Build extra “thick” copies with gentle jitter
    extraGeos.forEach((g, i) => {
      const amt = copyJitter * (i + 1)
      const jittered = jitterPoints(main, amt)
      setAndBound(g, jittered)
    })
  ...
  return (
    <group renderOrder={999} frustumCulled={false}>
      {/* main bolt twice (core + inner tint) */}
      <line>
      ...
      {/* extra jittered copies to fake thickness */}
      {extraGeos.map((g, i) => (
        <line key={i}>
          <primitive object={g} attach="geometry" />
          {/* dimmer towards the outside */}
          <primitive object={thinMat.clone()} attach="material" args={[]} />
        </line>
      ))}
