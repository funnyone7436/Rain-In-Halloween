  ...
  return (
    <group>
      {positions.map(([x, , z], i) => {
        ...
        return (
          <Billboard key={i} position={[x, y, z]} follow={true} lockZ={false}>
            <mesh>
...
function ImageRingPanel({ imgRing, setImgRing, panelKey }) {
  useControls(`Image Ring ${panelKey}`, {
    ...
    radius:       { value: imgRing.radius, min: 2, max: 80, step: 0.1, onChange: v => setImgRing(s => ({ ...s, radius: v })) },
    heightY:      { value: imgRing.y, min: -4, max: 60, step: 0.05, onChange: v => setImgRing(s => ({ ...s, y: v })) },
    size:         { value: imgRing.size, min: 0.2, max: 32, step: 0.05, onChange: v => setImgRing(s => ({ ...s, size: v })) },
...
export default function App() {
  ...
  function saveToTxt() {
    const settings = { sky, env, coaster, rain, imgRing, orbit: { autoRotate } }
  ...
  // your image paths under public/img
