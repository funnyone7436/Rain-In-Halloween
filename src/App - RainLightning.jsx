...
/* ==================== LIGHTNING: CLASSIC (seeded, no spaghetti) ==================== */
/* Local PRNG so rain (or anything else) can't affect the shape */
...
function enforceDownward(pts, minStep = 0.02) {
  ...
  for (let i = 1; i < out.length; i++) {
    if (out[i].y >= lastY - minStep) {
      out[i].y = lastY - (minStep + Math.random() * 0.04) // tiny variation ok
    }
    lastY = out[i].y
  }
  return out
...
function LightningClassic({
  ...
  minDur = 0.18,
  maxDur = 0.36,
  // shape (matches your earlier look)
  minVerticality = 0.98,
  minHeight = 10,
  maxHeight = 22,
  lateralJitter = 0.05,
  ...
  flashPower = 900,
  // determinism
  seed = 1337          // change this if you want different-looking bolts
}) {
  ...
  const flash = useRef()
  const state = useRef({ active: false, t: 0, dur: 0.2, cooldown: 0, strikeSeed: seed })
  ...
  const build = () => {
    // use local PRNG — nothing else can change the sequence
...
export default function App() {
  ...
  function saveToTxt() {
    const settings = { sky, env, coaster, rain, orbit: { autoRotate } }
  ...
  return (
    <>
      ...
      >
        ...
        {/* Seeded classic lightning (independent of rain) */}
        <LightningClassic
          enabled
          color="#ffffff"
          cloudY={20}
          ...
          power={8}
          flashPower={900}
          seed={1337}         // change to get a different look
        />
        <Rain
