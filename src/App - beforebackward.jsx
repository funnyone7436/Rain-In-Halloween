...
// movement calc (drop-in)
const onResults = React.useCallback((res) => {
  const lm = res.poseLandmarks
  const now = performance.now()

  // keep a tiny state machine inside prevLmRef so we don't add new refs
  let st = prevLmRef.current
  if (!st || typeof st !== 'object' || st.__v !== 1) {
    st = prevLmRef.current = { __v: 1, y: null, t: 0, g: null, out: 0, last: 0 }
  }

  // no landmarks → decay output a bit
  if (!lm) {
    st.out *= 0.9
    onChange?.(st.out)
    return
  ...
  // average of visible hips (23,24). y is normalized (top=0, bottom=1)
  const hips = [23, 24]
    .map(i => lm[i])
    .filter(p => p && (p.visibility ?? 0) >= 0.6)
  ...
  const y = (hips[0].y + (hips[1]?.y ?? hips[0].y)) / (hips[1] ? 2 : 1)

  // --- light throttle to ~15 fps to cut work on slow devices ---
  ...
  // upward velocity (positive when going up since y decreases upward)
  const vyUp = Math.max(0, (st.y - y) / dt)   // units: norm/sec

  // slowly-tracked ground (follow downward quickly, upward very slowly)
  const targetG = Math.max(y, st.g ?? y)      // never jump ground upward
  st.g = (st.g ?? y) * 0.99 + targetG * 0.01  // tweak the 0.99/0.01 to taste

  // height above ground (bigger when you jump)
  const lift = Math.max(0, st.g - y)          // units: normalized

  // simple linear metric (no 0..1 clamp). tune these three:
  const GAIN_V = 900   // how much speed matters
  const GAIN_H = 450   // how much height matters
  const SMOOTH = 0.35  // 0 = raw/noisy, 0.25 = a bit smoother

  const raw = GAIN_V * vyUp + GAIN_H * lift
  st.out = st.out * (1 - SMOOTH) + raw * SMOOTH
...
function MoonSystem({
  ...
  ringTextures = [`${BASE}img/w1.png`, `${BASE}img/w2.png`, `${BASE}img/w4.png`, `${BASE}img/w5.png`, `${BASE}img/w6.png`, `${BASE}img/w7.png`],
  // world orbit
  moonOrbitRadius = 36,
  moonHeight = 18,
  moonOrbitSpeed = 0.06,
  // BASE values (used when there's no movement)
  moonSize = 3,
  ringRadius = 56,
  ringSpinSpeed = 0.4,
  ...
  // NEW: motion targets + smoothing
  moonSizeTarget,             // e.g. dynamicMoonSize
  ringSpinSpeedTarget,        // e.g. dynamicRingSpin
  smooth = 0.18               // 0..1-ish; higher = snappier
}) {
  ...
  // ---- SMOOTHING STATE ----
  const sizeCurr = useRef(moonSize)                 // current (smoothed) size
  const spinCurr = useRef(ringSpinSpeed)            // current (smoothed) spin
  const sizeTgt  = useRef(moonSizeTarget ?? moonSize)
  const spinTgt  = useRef(ringSpinSpeedTarget ?? ringSpinSpeed)

  useEffect(() => { sizeTgt.current = (moonSizeTarget ?? moonSize) }, [moonSizeTarget, moonSize])
  useEffect(() => { spinTgt.current = (ringSpinSpeedTarget ?? ringSpinSpeed) }, [ringSpinSpeedTarget, ringSpinSpeed])
  ...
  useFrame(({ clock }, dtRaw) => {
    ...
    // dt-aware smoothing (exponential)
    const dt = Math.min(dtRaw, 0.05)
    ...
    // spin ring with smoothed speed
    if (ringRef.current) ringRef.current.rotation.y = -t * spinCurr.current
  })
...
// === Motion → Moon params ===
	const MOON_BASE = 3;      // current default
	const MOON_MAX  = 6;     // cap (tweak)
	const SPIN_BASE = 0.06;    // current default
	const SPIN_MAX  = 0.1;    // cap (tweak)

	const { dynamicMoonSize, dynamicRingSpin } = React.useMemo(() => {
	  const DEAD = 20;                               // ignore tiny movement
	  const m = Math.max(0, motionValue - DEAD);     // movement beyond dead-zone

	  // 0..1 response that rises fast then levels off (nice & stable)
	  const t = 1 - Math.exp(-m / 1000);              // tweak 120 for sensitivity

	  return {
		dynamicMoonSize: THREE.MathUtils.lerp(MOON_BASE, MOON_MAX, t),
		dynamicRingSpin: THREE.MathUtils.lerp(SPIN_BASE, SPIN_MAX, t),
	  }
	}, [motionValue])
